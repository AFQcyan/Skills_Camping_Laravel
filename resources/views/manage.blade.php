...
if('000-0000-0000' != $_SESSION['user']['phone'] && '관리자' != $_SESSION['user']['name']) {
    msgAndGo( '관리자만 이용할 수 있습니다.','./login.php');
}
...
$rvList = DB::fetchAll("SELECT * FROM reservation");
...
$reservationList = DB::fetchAll("SELECT * FROM reservation WHERE date>=? AND date<=? ORDER BY id DESC", [$startDate, $endDate]);
?>
<section id="reserv" class='manage'>
    ...
    <div class="content">
        ...
        <div class="date">
            <p>시작일 : <input type="date" name="startdate" value="<?=p($startDate)?>"></p>
            <p>종료일 : <input type="date" name="enddate" value="<?=p($endDate)?>"></p>
        ...
        <table border="1">
            ...
            if($page == 0){
                ...
                <tr>
                    ...
                    <td><?= p($resv->name) ?></td>
                    <td><?= p($resv->phone) ?></td>
                    <td><?= p($resv->type=='ongoing' ? '예약중' : '예약완료') ?></td>
                    <td><?= p($resv->create_date) ?></td>
                    <td>
                        <?php if($resv->type == 'ongoing'): ?>
                            <button class="title-btn" onclick = 'location.href="./process_reservation_manage.php?id=<?=p($resv->id)?>&process=accept"'>승인</button>
                            <button class="title-btn" onclick = 'location.href="./process_reservation_manage.php?id=<?=p($resv->id)?>&process=reject"'>취소</button>
                        <?php else: ?>
                            이미 승인됨
                        <?php endif; ?>
                    </td>
...
<section id="order" class="manage">
    ...
    <div class="content">
        ...
        <?php
        $order = DB::fetchAll("SELECT * FROM orders ORDER BY id DESC");
        $reservation = DB::fetchAll("SELECT * FROM reservation ORDER BY id DESC");
        ?>
    ...
    <script>
        ...
        function createTr({id, date, place, order, create_date}) {
        ...
        function setTotal(order) {
            document.querySelector('.all-order-count').innerHTML = currentReservationList.reduce((s, {order})=>
                    s+order.reduce((s,order)=>(order.type !== 'cancel' ? s+1 : s), 0)
                , 0)

            document.querySelector('.all-order-cancel').innerHTML = currentReservationList.reduce((s, {order})=>
                    s+order.reduce((s,order)=>(order.type === 'cancel' ? s+1 : s), 0)
                , 0)

            document.querySelector('.all-order-complete-count').innerHTML = currentReservationList.reduce((s, {order})=>
                    s+order.reduce((s,order)=>(order.type === 'complete' ? s+1 : s), 0)
                , 0)

            document.querySelector('.all-order-complete-price').innerHTML = parseInt(currentReservationList.reduce((s, {order})=>
                    (s+order.reduce((s,order)=>(order.type === 'complete' ? s+order.price : s), 0))
                , 0)).toLocaleString()
        }
    </script>
</section>
